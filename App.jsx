...
export default function App() {
  // PIN Login
  const [currentUser, setCurrentUser] = useState(() => sessionStorage.getItem("fc_user") || null);
  ...
  const [fab, setFab] = useState(false); // floating action button menu
  ...
  const [fabMode, setFabMode] = useState(null); // null | "tx" | "msg" // {id, type:"in"|"out", amount, note, cat, date, who}
  ...
  // Debounced save all state
  const allState = { tasks, places, bgt, moods, diary, fd, phase, phases, qa, rc, kd, goals, arrivalDate, themeId, shapeId, departed, savings, italyBgt };
  const doSave = useCallback(() => {
    if (!ok || skipSync.current) return;
    saveKey('all', allState);
  }, [ok, ...Object.values(allState), saveKey]);
  useEffect(() => { clearTimeout(sr.current); sr.current = setTimeout(doSave, 600); }, [doSave]);

  // Save chat to Supabase (individual messages)
  const sendChatToDb = useCallback(async (msg) => {
    try {
      await supabase.from('chat_messages').insert({ who: msg.who, text: msg.text || '', attach: msg.attach || null });
    } catch {}
  }, []);
  ...
  const sendTxToDb = useCallback(async (tx) => {
    try {
      await supabase.from('transactions').insert({ type: tx.type, amount: tx.amount, note: tx.note || '', cat: tx.cat || '🛒', who: tx.who || currentUser || 'fabio' });
    } catch {}
  }, [currentUser]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (txData) setTxns(txData.map(t => ({ id: t.id, type: t.type, amount: Number(t.amount), note: t.note, cat: t.cat, who: t.who, date: new Date(t.created_at).toLocaleDateString("it-IT", { day: "numeric", month: "short" }) })));
    ...
    const channel = supabase.channel('famiglia')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'app_state', filter: 'key=eq.all' }, (payload) => {
        if (payload.new?.updated_by === currentUser) return; // skip own updates
        try {
          skipSync.current = true;
          const d = typeof payload.new.value === 'string' ? JSON.parse(payload.new.value) : payload.new.value;
          if (d.tasks) setTasks(d.tasks); if (d.places) setPlaces(d.places); if (d.bgt) setBgt(d.bgt);
          if (d.moods) setMoods(d.moods); if (d.diary) setDiary(d.diary);
          if (d.fd) setFd(d.fd); if (d.phase) setPhase(d.phase); if (d.phases) setPhases(d.phases);
          if (d.qa != null) setQa(d.qa); if (d.rc != null) setRc(d.rc); if (d.kd) setKd(d.kd);
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'transactions' }, (payload) => {
        ...
        setTxns(prev => {
          ...
          return [{ id: t.id, type: t.type, amount: Number(t.amount), note: t.note, cat: t.cat, who: t.who, date: new Date(t.created_at).toLocaleDateString("it-IT", { day: "numeric", month: "short" }) }, ...prev];
        });
      })
      .subscribe();
  ...
  const sendChat = () => { if (!chatTxt.trim() && !chatAttach) return; const msg = { id: uid(), who: chatWho, text: chatTxt, time: nowT(), date: nowD() }; if (chatAttach) msg.attach = chatAttach; setChat(p => [...p, msg]); sendChatToDb(msg); setChatTxt(""); setChatAttach(null); };
  ...
  if (!ok && currentUser) return <div style={{ fontFamily: "'Nunito',sans-serif", display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", fontSize: 24, background: T.bg, color: T.text }}>🇵🇱 Caricamento...</div>;
    ...
    return (
      <div style={{ fontFamily: "'Nunito',sans-serif", background: "linear-gradient(135deg,#F5F3EE,#FFF5F0)", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 30 }}>
          {FAM.map(f => (
            <div key={f.id} style={{ textAlign: "center", opacity: 0.5 }}>
              <div style={{ width: 36, height: 36, borderRadius: 13, background: f.grad, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>{f.emoji}</div>
              <div style={{ fontSize: 8, fontWeight: 700, color: "#B0ADA6", marginTop: 2 }}>{f.name}</div>
            </div>
          ))}
        </div>
        {/* Logout info */}
        <div style={{ marginTop: 20, fontSize: 10, color: "#B0ADA6", textAlign: "center" }}>
          Fabio: 1226 · Lidia: 2808<br/>Walter: 1510 · Lucrezia: 2301
        </div>
      </div>
    );
  }
    ...
    <div style={{ fontFamily: F1, background: T.bg, minHeight: "100vh", maxWidth: 480, margin: "0 auto", paddingBottom: 90, color: T.text, transition: "background .3s, color .3s" }}>
      ...
      {showSettings && <div style={{ margin: "12px 12px 0" }}>
        <Card style={{ border: `2px solid ${pc}20` }}>
          ...
          <button onClick={() => { sessionStorage.removeItem("fc_user"); setCurrentUser(null); setPin(""); }} style={{ width: "100%", marginTop: 6, padding: 10, borderRadius: S.btn, background: "transparent", color: "#E25555", border: `2px solid #E2555530`, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: F1 }}>🚪 Cambia Utente ({FAM.find(f=>f.id===currentUser)?.name})</button>
        </Card>
      </div>}
                ...
                <div style={{ padding: 20, textAlign: "center", background: "linear-gradient(135deg,#FF6B3510,#F5A62308)" }}>
                  ...
                  <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
                    <button onClick={() => {
                      ...
                      const pm = { id: uid(), who: currentUser || "fabio", text: "✈️ SIAMO PARTITI! Poznań, arriviamo! 🇵🇱", time: nowT(), date: nowD() }; setChat(p => [...p, pm]); sendChatToDb(pm);
          ...
          <Card>
            ...
            {txOpen && <div>
              ...
              <button onClick={() => {
                ...
                if (!amt || amt <= 0) return;
                const tx = { id: uid(), type: txType, amount: amt, note: txNote, cat: txCat, date: nowD(), who: currentUser || "fabio" };
                setTxns(p => [tx, ...p]); sendTxToDb(tx);
      ...
      {fab && <div onClick={() => { setFab(false); setFabMode(null); }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 200, backdropFilter: "blur(3px)" }} />}
      {fab && <div style={{ position: "fixed", bottom: 90, left: "50%", transform: "translateX(-50%)", width: "calc(100% - 24px)", maxWidth: 456, zIndex: 210 }}>
        <div className="fu" style={{ background: T.card, borderRadius: S.card, padding: 16, boxShadow: `0 8px 40px ${T.dark ? "rgba(0,0,0,0.5)" : "rgba(0,0,0,0.15)"}` }}>
          {/* Mode selector */}
          {!fabMode && <div>
            <div style={{ fontFamily: F2, fontSize: 16, fontWeight: 900, marginBottom: 12, textAlign: "center" }}>Azione Rapida</div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => { setFabMode("tx"); setFabTxType("out"); }} className="ch" style={{ flex: 1, padding: "16px 8px", borderRadius: S.card, background: "linear-gradient(135deg,#E2555510,#FF7B7B08)", border: "2px solid #E2555525", cursor: "pointer", textAlign: "center" }}>
                <div style={{ fontSize: 28 }}>📤</div>
                <div style={{ fontSize: 13, fontWeight: 800, color: "#E25555", marginTop: 4 }}>Uscita</div>
              </button>
              <button onClick={() => { setFabMode("tx"); setFabTxType("in"); }} className="ch" style={{ flex: 1, padding: "16px 8px", borderRadius: S.card, background: "linear-gradient(135deg,#6BC98610,#47C5D808)", border: "2px solid #6BC98625", cursor: "pointer", textAlign: "center" }}>
                <div style={{ fontSize: 28 }}>📥</div>
                <div style={{ fontSize: 13, fontWeight: 800, color: "#6BC986", marginTop: 4 }}>Entrata</div>
              </button>
              <button onClick={() => setFabMode("msg")} className="ch" style={{ flex: 1, padding: "16px 8px", borderRadius: S.card, background: `linear-gradient(135deg,${pc}10,${pc}05)`, border: `2px solid ${pc}25`, cursor: "pointer", textAlign: "center" }}>
                <div style={{ fontSize: 28 }}>💬</div>
                <div style={{ fontSize: 13, fontWeight: 800, color: pc, marginTop: 4 }}>Messaggio</div>
              </button>
            </div>
          </div>}
          ...
          {fabMode === "tx" && <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              ...
              <div style={{ display: "flex", gap: 4 }}>
                <button onClick={() => setFabTxType("out")} className="ch" style={{ padding: "5px 12px", borderRadius: S.btn, border: fabTxType === "out" ? "2px solid #E25555" : `2px solid ${T.border}`, background: fabTxType === "out" ? "#E2555508" : T.card, cursor: "pointer", fontSize: 12, fontWeight: 800, color: fabTxType === "out" ? "#E25555" : T.muted, fontFamily: F1 }}>📤 Uscita</button>
                <button onClick={() => setFabTxType("in")} className="ch" style={{ padding: "5px 12px", borderRadius: S.btn, border: fabTxType === "in" ? "2px solid #6BC986" : `2px solid ${T.border}`, background: fabTxType === "in" ? "#6BC98608" : T.card, cursor: "pointer", fontSize: 12, fontWeight: 800, color: fabTxType === "in" ? "#6BC986" : T.muted, fontFamily: F1 }}>📥 Entrata</button>
              </div>
            </div>
            <div style={{ position: "relative", marginBottom: 8 }}>
              <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", fontSize: 18, fontWeight: 900, color: fabTxType === "out" ? "#E25555" : "#6BC986" }}>€</span>
              <input type="number" value={fabAmt} onChange={e => setFabAmt(e.target.value)} placeholder="0" autoFocus style={inp({ paddingLeft: 36, fontSize: 24, fontWeight: 900, fontFamily: F2, borderColor: fabTxType === "out" ? "#E2555540" : "#6BC98640", textAlign: "right", padding: "12px 14px 12px 36px" })} />
            </div>
            <div style={{ display: "flex", gap: 3, marginBottom: 8 }}>
              {(fabTxType === "out" ? [5, 10, 20, 50, 100] : [100, 500, 900, 1400]).map(v => (
                <button key={v} onClick={() => setFabAmt(String(v))} className="ch" style={{ flex: 1, padding: "5px 2px", borderRadius: S.btn, border: `1.5px solid ${T.border}`, background: fabAmt === String(v) ? `${pc}08` : T.card, cursor: "pointer", fontSize: 11, fontWeight: 700, color: fabAmt === String(v) ? pc : T.muted, fontFamily: F1 }}>{v}</button>
              ))}
            </div>
            <div style={{ display: "flex", gap: 3, flexWrap: "wrap", marginBottom: 8 }}>
              {TX_CATS.slice(0, 8).map(([ic, lb]) => (
                <button key={ic} onClick={() => setFabCat(ic)} className="ch" style={{ padding: "4px 7px", borderRadius: S.btn, border: fabCat === ic ? `2px solid ${pc}` : `1.5px solid ${T.border}`, background: fabCat === ic ? `${pc}08` : T.card, cursor: "pointer", fontSize: 12 }}>
                  {ic}
                </button>
              ))}
            </div>
            ...
            <button onClick={() => {
              const amt = parseFloat(fabAmt);
              if (!amt || amt <= 0) return;
              const tx = { id: uid(), type: fabTxType, amount: amt, note: fabNote, cat: fabCat, date: nowD(), who: currentUser || "fabio" };
              setTxns(p => [tx, ...p]); sendTxToDb(tx);
              setFabAmt(""); setFabNote(""); setFab(false); setFabMode(null);
            }} style={{ width: "100%", padding: 12, borderRadius: S.btn, background: fabTxType === "out" ? "linear-gradient(135deg,#E25555,#FF7B7B)" : "linear-gradient(135deg,#6BC986,#47C5D8)", color: "#fff", border: "none", fontSize: 15, fontWeight: 800, cursor: "pointer", fontFamily: F1 }}>
          ...
          {fabMode === "msg" && <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <button onClick={() => setFabMode(null)} style={{ background: "none", border: "none", fontSize: 14, cursor: "pointer", color: T.muted }}>← Indietro</button>
              <span style={{ fontFamily: F2, fontSize: 14, fontWeight: 700 }}>💬 Messaggio</span>
            </div>
            <div style={{ display: "flex", gap: 3, marginBottom: 8 }}>{FAM.map(f => <button key={f.id} onClick={() => setChatWho(f.id)} className="ch" style={{ flex: 1, padding: "6px 2px", borderRadius: S.btn, border: chatWho === f.id ? `2.5px solid ${f.color}` : `2.5px solid ${T.border}`, background: chatWho === f.id ? `${f.color}08` : T.card, cursor: "pointer", fontSize: 18 }}>{f.emoji}</button>)}</div>
            <div style={{ display: "flex", gap: 6 }}>
              <input value={fabMsg} onChange={e => setFabMsg(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && fabMsg.trim()) { const m={id:uid(),who:chatWho||currentUser,text:fabMsg,time:nowT(),date:nowD()};setChat(p=>[...p,m]);sendChatToDb(m); setFabMsg(""); setFab(false); setFabMode(null); } }} placeholder="Scrivi..." autoFocus style={inp({ borderRadius: S.card, flex: 1 })} />
              <button onClick={() => { if (fabMsg.trim()) { const m={id:uid(),who:chatWho||currentUser,text:fabMsg,time:nowT(),date:nowD()};setChat(p=>[...p,m]);sendChatToDb(m); setFabMsg(""); setFab(false); setFabMode(null); } }} style={{ width: 44, height: 44, borderRadius: 22, background: FAM.find(f => f.id === (chatWho||currentUser))?.grad, color: "#fff", border: "none", fontSize: 18, cursor: "pointer", flexShrink: 0 }}>↑</button>
            </div>
          </div>}
        </div>
      </div>}
      ...
      {/* FAB Button */}
      <button onClick={() => { setFab(!fab); if (fab) setFabMode(null); }} className="ch" style={{ position: "fixed", bottom: 68, right: "max(12px, calc((100vw - 480px)/2 + 12px))", width: 52, height: 52, borderRadius: 26, background: fab ? T.muted : "linear-gradient(135deg,#FF6B35,#F5A623)", color: "#fff", border: "none", fontSize: fab ? 18 : 24, cursor: "pointer", zIndex: 220, boxShadow: fab ? "none" : "0 4px 20px rgba(245,166,35,0.4)", transition: "all .2s", display: "flex", alignItems: "center", justifyContent: "center" }}>{fab ? "✕" : "+"}</button>
